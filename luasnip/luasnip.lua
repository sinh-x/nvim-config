...
ls.add_snippets('all', {
  ...
  s('trig', {
    ...
    end, { 1 }, { user_args = { 'Will be appended to text from i(0)', 'aabb' } }),
    i(0),
  }),
  s('trig2', {
...
ls.config.setup {
  load_ft_func =   -- Also load both lua and json when a markdown-file is opened,
  -- javascript for html.
...
ls.env_namespace('POS', {
  init = function(pos)
    return { VAL = vim.inspect(pos) }
  end,
})
...
ls.add_snippets('all', {
  ...
  s(
    ...
    f(function(args, snip)
      ...
      for _, ele in ipairs(env.SELECT_RAW) do
